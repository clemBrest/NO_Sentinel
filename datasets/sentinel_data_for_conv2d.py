...
class SentinelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        i = i+self.kernel_size // 2 if i < self.kernel_size // 2 else i
        i = i-self.kernel_size // 2 if i > self.shape[-2] - self.kernel_size // 2 else i

        j = j+self.kernel_size // 2 if j < self.kernel_size // 2 else j
        j = j-self.kernel_size // 2 if j > self.shape[-1] - self.kernel_size // 2 else j

        i_slice = slice(i-self.kernel_size // 2, i+self.kernel_size // 2 + 1)
        j_slice = slice(j-self.kernel_size // 2, j+self.kernel_size // 2 + 1)  
        
        tar_index = slice(t + 1, t + self.future + 1)
        tarm1_index = slice(t, t + self.future)      

        inpm1 = (self.sentinel_data[t-1, :, i_slice, j_slice])
        inp0 = (self.sentinel_data[t, :, i_slice, j_slice])


        tarm1 = (self.sentinel_data[tarm1_index, :,i, j])
